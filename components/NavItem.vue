<template>
  ...
  >
    ...
    <ul
      ...
      :class="{
        'lg:absolute lg:top-full lg:start-0 lg:flex-row lg:w-max xl:ltr:-translate-x-[50px] xl:rtl:translate-x-[50px] lg:translate-y-px lg:py-8 lg:px-4 ms-4 xl:rounded-b-lg lg:invisible lg:group/edit lg:group-hover/item:visible shadow-md':
          layout === 'header',
        '': layout === 'footer',
      }"
    >
      <li v-for="(child, index) in children" :key="index">
        <ul
          v-if="child.sections && child.sections.length"
          class="flex flex-col"
          :class="{
            'lg:flex-row gap-6 lg:gap-8': layout === 'header',
            'gap-2.5': layout === 'footer',
          }"
        >
          <li
            v-for="(section, sectionIndex) in child.sections"
            :key="sectionIndex"
          >
            <span
              v-if="
            ...
            <ul
              v-if="section.pages && section.pages.length"
              :class="{
                'mt-2 lg:mt-8': section.label && layout === 'header',
                'gap-1.5 lg:gap-4': layout === 'header',
                'gap-2.5': layout === 'footer',
              }"
              class="flex flex-col"
            >
              ...
              >
                <NuxtLinkLocale
                  ...
                  exact-active-class="active"
                  :class="{
                    'nav-link-color link-color flex justify-between items-center gap-6 py-2 px-4 rounded-sm':
                      layout === 'header',
                  }"
                >
