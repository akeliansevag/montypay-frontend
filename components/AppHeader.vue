<template lang="">
  <header
    :class="$route.path === '/' || $route.path === '/ar' ? 'fixed' : 'sticky'"
    class="top-0 start-0 w-full z-30 transition-all duration-300 ease-in-out shadow-md"
  >
    <div
      class="w-full z-[3] border-b border-[#e9edf422]"
      :class="navVisible ? 'fixed' : ''"
    >
      <div class="container flex justify-between items-center">
        <NuxtLinkLocale to="/" class="nav-link me-4">
          <AppLogo
            :fill="
              navVisible || $route.path === '/' || $route.path === '/ar'
                ? '#FFFFFF'
                : 'auto'
            "
          />
        ...
        <nav aria-label="Primary" class="nav hidden lg:block">
          <NavItemsWrapper layout="header" class="flex max-xl:gap-4 xl:gap-7" />
        </nav>
        <div class="flex items-center gap-2">
          <ul class="flex gap-2 max-lg:hidden">
            <li class="flex items-center nav-link">
              <NuxtLinkLocale
                class="nav-link bg-secondary drop-shadow-lg rounded-lg px-2 ltr:py-2 rtl:py-[5px] text-nowrap"
                to="https://portal.montypay.com/"
                target="_blank"
              >
                <h5 class="nav-link text-[15px]">
                  {{ t("General.Buttons.Log In") }}
                </h5>
              </NuxtLinkLocale>
            </li>
          </ul>
            ...
            >
              ...
              >
                <div
                  ...
                  class="px-8 py-2 font-semibold text-black cursor-pointer hover:bg-gray-200"
                  @click="changeLocale('en')"
                >
                  EN
                </div>
                <div
                  :class="locale == 'ar' ? 'bg-gray-200' : ''"
                  class="px-8 py-2 font-semibold text-black cursor-pointer hover:bg-gray-200"
          ...
          <div
            @click="toggleNav"
            class="menu-btn block lg:hidden pt-1.5 pb-2.5 text-3xl cursor-pointer"
          >
            <Icon
              v-if="!navVisible"
              name="fa7-solid:bars"
            ...
            <Icon
              v-if="navVisible"
              name="fa7-solid:x"
              class="icon text-lg text-primary"
            />
          </div>
        </div>
      </div>
    ...
    <nav
      aria-label="Primary"
      class="nav block lg:hidden fixed top-0 start-0 h-screen w-full text-primary bg-quaternary transition-transform duration-300 ease-in-out z-[2]"
      :class="{ 'ltr:-translate-x-full rtl:translate-x-full': !navVisible }"
    >
      <div class="container">
